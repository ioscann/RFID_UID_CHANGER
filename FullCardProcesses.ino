...
void ReadDataFromBlock(int blockNum, byte readBlockData[]) // Karttaki veriyi okuma fonksiyonu
{
  ...
  mfrc522.PICC_HaltA(); // Yeni bir kart okutulana kadar çalışmayı durdurur
}

void printData() {for (int j=0 ; j<16 ; j++) {Serial.write(readBlockData[j]);}} // Kartın istenilen blokundaki veriyi yazdıran fonksiyon
...
{ 
  ...
  if (Serial.available())
  {
    ...
    if (data == '0') {readUID(); Serial.flush(); goto start;}
    else if (data == '1') {delay(2000); int A = Serial.parseInt(); delay(2000); int B = Serial.parseInt(); delay(3000); int C = Serial.parseInt(); delay(3000); int D = Serial.parseInt(); delay(3000); setNewUID(A,B,C,D); delay(2000); goto start;}
    else if (data == '2') { delay(2000); int blockNum = Serial.parseInt(); delay(2000); String data = Serial.readString(); delay(3000); WriteDataToBlockString(blockNum,data); Serial.flush(); goto start;}
    else if (data == '3') { int blockNum = Serial.parseInt(); ReadDataFromBlock(blockNum,readBlockData); delay(500); printData(); Serial.flush(); goto start;}
  }   
}